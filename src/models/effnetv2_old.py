...
class EffnetV2_Key_Frame(torch.nn.Module):
    ...
    def mask_sequence(self, padded_seq, org_seq_len):  
        mask = torch.zeros(padded_seq.size()[:2], dtype=torch.uint8)
    ...
    def forward(self, x):
        ...
        mask = self.mask_sequence(features_padded, org_seq_len = org_seq_len).cuda() if torch.cuda.is_available() else self.mask_sequence(features_padded, org_seq_len = org_seq_len)

        
        if self.use_key_frame_attention:
            x = self.key_frame_attention(features_padded.permute(0, 2, 1), org_seq_len = org_seq_len, Mask = mask)
...
test_tensor = torch.rand(2, 3, 1, 512, 512)
