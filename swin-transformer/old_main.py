...
def main(config):
    ...
    if config.MODEL.RESUME:
        ...
        if config.MODEL.TASK_TYPE == 'cls':
            ...
            print('Validating model after loading checkpoint...')
            mae_meter, mape_meter, rmse_meter, loss_meter_reg = validate(config, data_loader_val, model, experiment)
    ...
    if config.EVAL_MODE == False:
        for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
            ...
            validate(config, data_loader_val, model, experiment)
...
def train_one_epoch(config, model, criterion_cls, criterion_reg, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, experiment):
    ...
    for idx, (images, indexes, Class, video, measure, ps, frames_n, measure_normalized) in enumerate(data_loader): ## changed
        ...
        if config.MODEL.TASK_TYPE == 'cls':
            ...
            measure_normalized = measure_normalized.unsqueeze(0).cuda(non_blocking=True)
            measure_normalized = measure_normalized.reshape(-1, 1)
            reg_loss = criterion_reg(outputs[1], measure_normalized)
            cls_loss = criterion_cls(outputs[0], Class)
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            if config.MODEL.TASK_TYPE == 'cls' or config.MODEL.TASK_TYPE == 'cls_reg':
                experiment.log_metric("Train loss_cls", loss_meter_cls.val)
            elif config.MODEL.TASK_TYPE == 'reg' or config.MODEL.TASK_TYPE == 'cls_reg':
                experiment.log_metric("Train loss_reg", loss_meter_reg.val)
...
def validate(config, data_loader, model, experiment):
    ...
    for idx, (images, indexes, Class, video, measure, ps, frames_n, measure_normalized) in enumerate(data_loader):
        ...
        if config.MODEL.TASK_TYPE == 'cls':
            ...
            print('Shape of measure: ', measure.shape)
    ...
    if config.MODEL.TASK_TYPE == 'cls':
        ...
        experiment.log_metric('Val acc1', acc1_meter.avg)
        experiment.log_metric('Val f1_score', f1_score_meter.avg)
        experiment.log_metric('Val recall', recall_meter.avg)
        experiment.log_metric('Val precision', precision_meter.avg)
    elif config.MODEL.TASK_TYPE == 'reg':
        print('Finished validation! Results:')
