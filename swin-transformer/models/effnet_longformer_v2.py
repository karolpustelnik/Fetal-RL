    ...
    def _construct_network(self, out_features = 7, task_type = 'reg'):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        self.backbone = EffNet(use_pretrained = False, out_features = out_features, freeze = True, unfreeze_last_layers = True).to('cuda:0', dtype=torch.float32)
...
print(model((test_tensor, positions)).shape)
