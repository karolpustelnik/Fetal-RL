...
class EffnetEtE(torch.nn.Module):
    def __init__(self, out_features = 7, in_channels = 1):
        ...
        self.detector = efficientnet_v2_s(weights = 'EfficientNet_V2_S_Weights.IMAGENET1K_V1')
        self.detector.features[0] = torch.nn.Conv2d(self.in_channels, 24, kernel_size=(3, 3), 
                                                    stride=(2, 2), padding=(1, 1), bias=False)
        self.detector.classifier = torch.nn.Identity()
        self.regressor = nn.Sequential(
            nn.LayerNorm(1280),
            nn.Linear(1280, 512),
            nn.GELU(),
            nn.Linear(512, 1))
        self.classifier = torch.nn.Sequential(nn.Linear(1280, self.out_features))
